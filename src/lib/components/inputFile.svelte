<script lang="ts">
	...
	const removeFile = async (fileName: string) => {
		untrack(() =>
			uploadedVideoFiles.update((videoFiles) =>
				videoFiles.filter((videoFile) => videoFile.name !== fileName)
			)
		);
		untrack(() =>
			unknownFiles.update((unknownFiles) =>
				unknownFiles.filter((unknownFile) => unknownFile !== fileName)
			)
		);
		try {
